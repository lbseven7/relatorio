...
def editar_servico():
    if not st.session_state.data.empty:
        id_editar = st.selectbox("Selecione o ID do Serviço para Editar", ["Selecione"] + list(st.session_state.data['ID']), key="id_editar")
        ...
        if id_editar != "Selecione":
            ...
            empresa = st.selectbox("Escolha a Empresa", ["Selecione"] + empresas_disponiveis, 
            ...
            servico = st.selectbox("Escolha o Serviço", ["Selecione"] + servicos_disponiveis, 
                                   index=servico_index, 
                                   key="servico_editar")
            setor = st.selectbox("Escolha o Setor", ["Selecione"] + setores_disponiveis, 
                                 index=setores_disponiveis.index(registro['Setor']) if registro['Setor'] in setores_disponiveis else 0, 
                                 key="setor_editar")
            data = st.date_input("Data", value=pd.to_datetime(registro['Data']), key="data_editar")
            quantidade = st.number_input("Quantidade", min_value=1, step=1, value=registro['Quantidade'], key="quantidade_editar")
...
def excluir_servico():
    if not st.session_state.data.empty:
        id_excluir = st.selectbox("Selecione o ID do Serviço para Excluir", ["Selecione"] + list(st.session_state.data['ID']), key="id_excluir")
...
def consultar_quantidade_servicos():
    servico_selecionado = st.selectbox("Selecione o Serviço", ["Selecione"] + servicos_disponiveis, key="servico_consulta")
...
def consultar_quantidade_servicos_empresa():
    empresa_selecionada = st.selectbox("Selecione a Empresa", ["Selecione"] + empresas_disponiveis, key="empresa_consulta")
    ...
    if st.button("Consultar Quantidade por Empresa"):
        ...
        if not df_resultados.empty:
            ...
            # Gerar relatório PDF
            if st.button("Emitir Relatório em PDF", key="relatorio_empresa"):
...
with aba[4]:
    ...
    empresa_selecionada = st.selectbox("Selecione a Empresa", ["Selecione"] + empresas_disponiveis, key="empresa_relatorio")
